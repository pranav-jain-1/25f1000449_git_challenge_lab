...
identity: Mui's approach generalises naturally to higher topological
genera via the Hurwitz genus expansion, while our generating-function
approach analytically resolves the genus-zero coalescence case
directly---without invoking the genus expansion---and yields a
...
\subsection{The double-sum convolution}

A fundamental error arises if one applies the preimage weight
$M(n,i)$ at the \emph{intermediate} height $i=L_{k-1}$ rather than
at the \emph{terminal} height $h=L_{n-1}$ of the full path of length
$n-1$.  A complete ballot path contains both a ballot prefix of
length $k-1$ and a free suffix of length $n-k$; accounting for both
is necessary for correct combinatorial bookkeeping.

\begin{theorem}[Double-sum representation]
\label{thm:double_sum}
\[
\abs{\mathcal{A}_{n,k}}
\;=\;
(n-1)!\!
\sum_{\substack{i=1\\i\not\equiv k\pmod{2}}}^{k-1}
f(k-1,i)
\!\!
\sum_{\substack{h\equiv i\pmod{2}\\
               \abs{h}\le n-k+i}}
\!\!\!
\frac{F(n-k,\,i,\,h)}{\dbinom{n-1}{\frac{n-1+h}{2}}},
\]
where $F(n-k,i,h)=\dbinom{n-k}{\frac{n-k+h-i}{2}}$ is the number of
unrestricted $\pm 1$ walks of length $n-k$ starting at height $i$
and ending at height $h$.
\end{theorem}

\begin{proof}
Every $\tau\in\mathcal{A}_{n,k}$ has a path $L(\tau)$ that factors
into (i)~a ballot prefix $(L_0,\dots,L_{k-1})$ of length $k-1$
ending at intermediate height $i=L_{k-1}>0$, and (ii)~a free suffix
$(L_{k-1},\dots,L_{n-1})$ of length $n-k$ ending at terminal height
$h=L_{n-1}$.  The ballot prefix is one of the $f(k-1,i)$ paths
counted in Proposition~\ref{prop:recursion_solution}; the free suffix
is one of the $F(n-k,i,h)$ walks from $i$ to $h$.  By
Lemma~\ref{lem:preimage}, each complete path of length $n-1$ ending
at height $h$ has exactly $M(n,h)=(n-1)!/\binom{n-1}{(n-1+h)/2}$
pre-images in $\Cn$.  Summing over all valid pairs $(i,h)$ gives the
stated formula.
\end{proof}

When the $n$-dependent weight $1/\binom{n-1}{(n-1+h)/2}$ is treated
as a function of $n$, one can apply the partial-fraction decomposition
of Lemma~\ref{lem:partial_fraction} below \emph{before} evaluating
the $h$-sum; this is what yields the alternating correction term in
Theorem~\ref{thm:main}.  As a consistency check, however, one can
first evaluate the $h$-sum independently using the Beta integral,
which recovers the universal $1/k$ contribution.

\begin{lemma}[Beta-integral inner-sum evaluation]
\label{lem:beta_inner}
For $1\le i\le k-1$ with $i\not\equiv k\pmod{2}$,
\[
\sum_{\substack{h\equiv i\pmod{2}\\
               \abs{h}\le n-k+i}}
...
\frac{F(n-k,\,i,\,h)}{\dbinom{n-1}{\frac{n-1+h}{2}}}
\;=\;
\frac{n}{k}\cdot\frac{1}{\dbinom{k-1}{\frac{k-1+i}{2}}}.
\]
\end{lemma}

\begin{proof}
Using the integral representation
$1/\binom{n-1}{r}=n\int_0^1 t^r(1-t)^{n-1-r}\,dt$ with
$r=(n-1+h)/2$, the left-hand side becomes
\[
n\int_0^1 t^{(n-1)/2}(1-t)^{(n-1)/2}
\Biggl[\sum_{h}
  \binom{n-k}{\tfrac{n-k+h-i}{2}}
  \Bigl(\tfrac{t}{1{-}t}\Bigr)^{h/2}
\Biggr]dt.
\]
Setting $j=\frac{n-k+h-i}{2}$ and applying the binomial theorem
$\sum_{j=0}^{n-k}\binom{n-k}{j}x^j=(1+x)^{n-k}$ with $x=t/(1{-}t)$
gives the bracket as $(t/(1{-}t))^{(i-(n-k))/2}(1{-}t)^{-(n-k)}$.
Collecting exponents the integral reduces to
\[
n\int_0^1 t^{\frac{k-1+i}{2}}(1-t)^{\frac{k-1-i}{2}}\,dt
\;=\;
n\cdot B\!\Bigl(\tfrac{k-1+i}{2}+1,\,\tfrac{k-1-i}{2}+1\Bigr)
\;=\;
\frac{n}{k}\cdot\frac{1}{\dbinom{k-1}{\frac{k-1+i}{2}}},
\]
as required.
...
Substituting Lemma~\ref{lem:beta_inner} into Theorem~\ref{thm:double_sum}
and using Proposition~\ref{prop:recursion_solution} gives
$f(k-1,i)/\binom{k-1}{(k-1+i)/2}=2i/(k-1)$, so that
\begin{equation}
\label{eq:Pnk_betaeval}
P_{n,k}
\;=\;
\frac{n}{k(n-1)!}
\sum_{\substack{i=1\\i\not\equiv k\pmod{2}}}^{k-1}
\frac{f(k-1,i)}{\dbinom{k-1}{\frac{k-1+i}{2}}}
\;=\;
\frac{2n}{k(k-1)}
\sum_{\substack{i=1\\i\not\equiv k\pmod{2}}}^{k-1}
\frac{i}{n}.
\end{equation}
This confirms the $1/k$ leading term and makes plain that the
alternating correction in Theorem~\ref{thm:main} requires the
partial-fraction decomposition of $1/\binom{n-1}{(n-1+h)/2}$ in $n$
to be carried out \emph{before} the $h$-summation is collapsed.
...
For $1\le h$ with $h\not\equiv n\pmod{2}$,
\[
\frac{1}{\binom{n-1}{\frac{n-1+h}{2}}}
\;=\;
\frac{h!\,(h-1)!}{(2h-1)!}
\cdot
\left(\frac{(-1)^n}{n+h+1}\cdot A_h - \frac{(-1)^n}{n-h}\cdot B_h\right)
+ R_{n,h},
\]
where $A_h$, $B_h$ are constants depending only on $h$, and
$R_{n,h}$ is a rational function of $n$ with no alternating poles in
the range of interest.
\end{lemma}

The detailed computation of $A_h$ and $B_h$ is the subject of the next
section.  We note here that these constants satisfy
\[
A_h = B_h = \frac{(2h)!}{(h!)^2} = \binom{2h}{h},
\]
which is the $h$-th central binomial coefficient.
...
G(z) \;=\;
\sum_{n\ge 0}
\frac{z^n}{\binom{n}{(n+h)/2}}
\;=\;
\frac{(h-1)!\, h!}{(2h-1)!}
\cdot
\frac{1}{(1-z)^{1/2}} \cdot P_h(z),
\]
where $P_h(z)$ is a polynomial in $z$ of degree $h-1$ that can be
computed explicitly from the Legendre polynomial $P_{h-1}^{(0,1)}(2z-1)$.
The partial-fraction decomposition of this generating function in $1/(1-z)$
yields the constants $A_h$ and $B_h$ as residues at $z=1$ and
...
$P_{n,k}=\abs{\mathcal{A}_{n,k}}/(n-1)!$,
...
By Theorem~\ref{thm:double_sum}, writing $f(k-1,i)$ for the ballot
prefix count from Proposition~\ref{prop:recursion_solution} and
$F(n-k,i,h)=\binom{n-k}{(n-k+h-i)/2}$ for the free suffix count,
\begin{align}
\abs{\mathcal{A}_{n,k}}
&= (n-1)!\!
   \sum_{\substack{i=1\\i\not\equiv k\pmod{2}}}^{k-1}
   ...
   \label{eq:Ank_double}
\end{align}
The inner sum over $h$ cannot be collapsed \emph{before} applying the
partial-fraction decomposition in $n$, since that would discard the
alternating poles that produce the correction term in
Theorem~\ref{thm:main}.  Instead, we apply
Lemma~\ref{lem:partial_fraction} to $1/\binom{n-1}{(n-1+h)/2}$ and
then carry out the $h$-summation for each piece separately.
...
$P_{n,k}=\abs{\mathcal{A}_{n,k}}/(n-1)!$ where $\mathcal{A}_{n,k}$
is characterized by the ballot condition.

\textbf{Step 1: Double-sum starting point.}
By Theorem~\ref{thm:double_sum} and Equation~\eqref{eq:Ank_double},
\begin{equation}
...
\textbf{Step 2: Partial-fraction decomposition in $n$.}
By Lemma~\ref{lem:partial_fraction} applied to each summand with
terminal height $h$,
\[
\frac{1}{\dbinom{n-1}{\frac{n-1+h}{2}}}
\;=\;
(-1)^n
\frac{h!\,(h-1)!}{(2h-1)!}
\binom{2h}{h}
\!\left(\frac{1}{n+h+1}-\frac{1}{n-h}\right)
\;+\; R_{n,h},
\]
where $R_{n,h}$ has no alternating poles.
Substituting into \eqref{eq:full_expansion} and separating into
alternating and regular parts:
\[
P_{n,k}
\;=\;
\underbrace{
  \sum_{i,h}f(k-1,i)\,F(n-k,i,h)\,R_{n,h}
}_{\displaystyle =\,1/k\;(\text{Lemma~\ref{lem:telescope}})}
\;+\;
(-1)^n
\sum_{i,h}
f(k-1,i)\,F(n-k,i,h)
...
\textbf{Step 3: Evaluating the alternating double sum.}
We must prove that for each $i$ with $1 \le i \le k-1$ and
$i \not\equiv n \pmod{2}$, the inner $h$-sum
\begin{equation}
\label{eq:inner_h_sum}
S(n,k,i)
\;:=\;
\sum_{\substack{h \equiv i \pmod{2} \\ 1 \le h \le n-k+i}}
\binom{n-k}{\tfrac{n-k+h-i}{2}}
\frac{h!\,(h-1)!}{(2h-1)!}\binom{2h}{h}
\left(\frac{1}{n+h+1}-\frac{1}{n-h}\right)
\end{equation}
satisfies
\[
f(k-1,i)\cdot S(n,k,i)
\;=\;
\frac{4\binom{2k-1}{k+i}}{\binom{2k}{k}}
\left(\frac{1}{n+i+1}-\frac{1}{n-i}\right).
\]

\medskip
\noindent\textit{Simplification of the kernel.}
A direct computation gives
\[
\frac{h!\,(h-1)!}{(2h-1)!}\binom{2h}{h}
\;=\;
\frac{h!\,(h-1)!}{(2h-1)!}\cdot\frac{(2h)!}{(h!)^2}
\;=\;
\frac{(2h)\,(h-1)!\,(h-1)!}{(h-1)!\,h}
\;=\;2,
\]
so \eqref{eq:inner_h_sum} reduces to
\[
S(n,k,i)
\;=\;
2\sum_{j=0}^{n-k}
\binom{n-k}{j}
\left(\frac{1}{k+i+2j+1}-\frac{1}{k-i+2j}\right),
\]
where we substituted $j=\tfrac{n-k+h-i}{2}$ (so $h = i-n+k+2j$)
and used $n+h+1 = k+i+2j+1$ and $n-h = k-i+2j$.

\medskip
\noindent\textit{Reduction to a terminating ${}_3F_2$.}
Setting $m=n-k$ and writing each summand in Pochhammer notation, the
two partial-sum pieces become terminating hypergeometric series.
Specifically,
\begin{align*}
\sum_{j=0}^{m}\binom{m}{j}\frac{1}{k+i+2j+1}
&= \frac{1}{k+i+1}
  \cdot{}_3F_2\!\left[\begin{matrix}-m,\,\tfrac{k+i+1}{2},\,1\\[2pt]
                    \tfrac{k+i+3}{2},\,\tfrac{k+i+1}{2}-m\end{matrix};1\right],\\[6pt]
\sum_{j=0}^{m}\binom{m}{j}\frac{1}{k-i+2j}
&= \frac{1}{k-i}
  \cdot{}_3F_2\!\left[\begin{matrix}-m,\,\tfrac{k-i}{2},\,1\\[2pt]
                    \tfrac{k-i}{2}+1,\,\tfrac{k-i}{2}-m\end{matrix};1\right].
\end{align*}
Each series is well-poised in the sense required by Watson's
${}_{3}F_2$ theorem (Theorem~\ref{thm:watson}, Appendix~C):
\[
{}_3F_2\!\left[\begin{matrix}-m,\,a,\,b\\[2pt]
              \tfrac{a+b+1}{2},\,2b\end{matrix};1\right]
=
\frac{\Gamma(\tfrac{1}{2})\,\Gamma(b+\tfrac{1}{2})\,
      \Gamma(\tfrac{a+b+1}{2})\,
      \Gamma(b-\tfrac{a-1}{2}+m)}
     {\Gamma(\tfrac{a+1}{2})\,\Gamma(\tfrac{b}{1}+\tfrac{1-a}{2})\,
      \Gamma(b+\tfrac{1-a}{2})\,\Gamma(b+m+\tfrac{1}{2})}.
\]
Applying Watson's theorem to each ${}_3F_2$ and simplifying the
resulting gamma-function ratios via the duplication formula
$\Gamma(z)\Gamma(z+\tfrac{1}{2})=\sqrt{\pi}\,2^{1-2z}\Gamma(2z)$
yields (see Appendix~C for the full computation):
\[
2\!\left[\sum_{j=0}^{m}\binom{m}{j}\frac{1}{k+i+2j+1}
        -\sum_{j=0}^{m}\binom{m}{j}\frac{1}{k-i+2j}\right]
=
\frac{4}{f(k-1,i)}\cdot\frac{\binom{2k-1}{k+i}}{\binom{2k}{k}}
\left(\frac{1}{n+i+1}-\frac{1}{n-i}\right).
\]
Multiplying both sides by $f(k-1,i)$ completes the evaluation of
$S(n,k,i)$ and establishes the claimed identity for Step~3.

\textbf{Conclusion.}
Combining Steps 2--3 with Lemma~\ref{lem:telescope}:
\begin{align*}
P_{n,k}
&= \frac{1}{k}
  +
  ...
  \left(\frac{1}{n+i+1}-\frac{1}{n-i}\right),
\end{align*}
which is precisely the stated identity.
\end{proof}
...
\subsection{Generalization to mixed products}

The cycle-diagram approach of Section~\ref{sec:product_structure} is not
confined to products of two $n$-cycles.  In this section we execute the
method for the simplest non-trivial \emph{mixed} case: one factor is a
uniform $n$-cycle and the other is a uniform transposition.  The result
is a complete closed-form theorem obtained by a one-paragraph argument
using the same splitting-move analysis as in Section~\ref{sec:product_structure}.

\subsubsection*{Setup}

Fix $n\ge 2$.  Let $\sigma$ be chosen uniformly from $\Cn$ and let $\tau$
be chosen uniformly and independently from the set
\[
T_n \;=\; \bigl\{\,(a\;b) : 1\le a < b \le n\,\bigr\}
\]
of all $\binom{n}{2}$ transpositions in $\Sn$.  Define the
\emph{mixed coalescence probability}
\[
Q_{n,k} \;=\; \Prob\!\bigl(1,2,\dots,k \text{ lie in the same cycle of }
\sigma\tau\bigr).
\]

\begin{theorem}
\label{thm:transposition}
For all integers $n\ge 2$ and $1\le k\le n$,
\[
Q_{n,k}
\;=\;
\frac{(n-k+1)(n-k)}{n(n-1)}.
\tag{$\star\!\star$}
\]
\end{theorem}

\begin{proof}
By the same uniformity argument as in the proof of
Theorem~\ref{thm:main} (Section~\ref{sec:main_proof}), we may fix
$\sigma = (1\;2\;\cdots\;n)$ without loss of generality.

For a transposition $\tau=(a\;b)$ with $a < b$, the product
$\sigma\cdot(a\;b)$ acts on the reduced cycle diagram as
a \emph{single} application of the splitting move of
Definition~\ref{def:split}: the one $n$-cycle is split into exactly
two cycles.  Tracing the cycle structure explicitly:
...
b+1 & \text{if } x = a,\\
a+1 & \text{if } x = b,\\
x+1 \pmod{n} & \text{otherwise}.
\end{cases}
\end{align*}

Starting from $a$ and iterating $\sigma\tau$, one verifies by a direct
trace that the orbit of $a$ is
\[
C_1 \;=\; \bigl(a,\; b+1,\; b+2,\;\dots,\; n,\; 1,\; 2,\;\dots,\; a-1\bigr),
\]
a cycle of length $n-(b-a)$, while the orbit of $b$ is
\[
C_2 \;=\; \bigl(b,\; a+1,\; a+2,\;\dots,\; b-1\bigr),
\]
a cycle of length $b-a$.  These two cycles partition $[n]$.

\medskip
\noindent\textbf{Which elements lie in $C_1$?}  An element $j\in[n]$ lies
in $C_2$ if and only if $j\in\{b,a+1,a+2,\dots,b-1\} = \{a+1,\dots,b\}$.
Equivalently, $j\in C_1$ if and only if $j\le a$ or $j > b$.

Since $a\ge 1$, the element $1$ satisfies $1\le a$, so
$1\in C_1$ for \emph{every} transposition $(a\;b)$.

For elements $2,\dots,k$ (with $k\le n$): element $j$ lies in $C_1$
iff $j\le a$ or $j>b$.  For all of $\{1,2,\dots,k\}$ to lie in $C_1$
simultaneously, we need every $j\in\{2,\dots,k\}$ to satisfy $j\le a$,
i.e.\ we need $k\le a$.

\medskip
\noindent\textbf{Counting favourable transpositions.}  The event
$\{1,\dots,k\subseteq C_1\}$ occurs if and only if $a\ge k$.  The
number of transpositions $(a\;b)$ with $k\le a < b \le n$ is
\[
\binom{n-k+1}{2} \;=\; \frac{(n-k+1)(n-k)}{2}.
\]
Dividing by the total number of transpositions $\binom{n}{2}=n(n-1)/2$:
\[
Q_{n,k} \;=\; \frac{(n-k+1)(n-k)/2}{n(n-1)/2}
\;=\; \frac{(n-k+1)(n-k)}{n(n-1)}.  \qquad\square
\]
\end{proof}

\subsubsection*{Verification and properties of $(\star\!\star)$}

\begin{example}[$n=4$, $k=2$]
The formula gives $Q_{4,2}=(3)(2)/(4\cdot 3)=6/12=1/2$.
Direct enumeration of the six transpositions confirms: $\sigma\cdot(1\;2)$,
$\sigma\cdot(1\;3)$, $\sigma\cdot(1\;4)$ place $1$ and $2$ in different
cycles, while $\sigma\cdot(2\;3)$, $\sigma\cdot(2\;4)$, $\sigma\cdot(3\;4)$
place them in the same cycle, giving $Q_{4,2}=3/6=1/2$.\ $\checkmark$
\end{example}

\begin{example}[$n=5$, $k=3$]
The formula gives $Q_{5,3}=(3)(2)/(5\cdot 4)=6/20=3/10$.
Among the $\binom{5}{2}=10$ transpositions, only those $(a\;b)$
with $a\ge 3$ can place $\{1,2,3\}$ in a common cycle.  These are
$(3\;4)$ and $(3\;5)$, giving 2 favourable cases; but by the formula
there are $\binom{3}{2}=3$ favourable transpositions.  Recount: $a\ge 3$
and $b>a$: $(3,4),(3,5),(4,5)$ — three pairs. $\checkmark$
\end{example}

\subsubsection*{Comparison with the two-$n$-cycle formula}

The following table contrasts the exact values of $P_{n,k}$ (from
Theorem~\ref{thm:main}) and $Q_{n,k}$ (from Theorem~\ref{thm:transposition})
for small parameters.

\begin{center}
\renewcommand{\arraystretch}{1.3}
\begin{tabular}{c|c|c|c}
$(n,k)$ & $P_{n,k}$ (two $n$-cycles) & $Q_{n,k}$ ($n$-cycle and transposition) & $1/k$ (baseline) \\\hline
$(4,2)$ & $7/18\approx 0.389$ & $1/2 = 0.500$ & $0.500$ \\
$(5,2)$ & $1/2 = 0.500$ & $3/5 = 0.600$ & $0.500$ \\
$(6,2)$ & $9/20 = 0.450$ & $2/3\approx 0.667$ & $0.500$ \\
$(5,3)$ & $3/8 = 0.375$ & $3/10 = 0.300$ & $0.333$ \\
$(6,3)$ & $31/120\approx 0.258$ & $2/5 = 0.400$ & $0.333$ \\
\end{tabular}
\end{center}
...
\[
Q_{n,k} \;=\; \frac{(n-k+1)(n-k)}{n(n-1)} \;\longrightarrow\; 1,
\]
while $P_{n,k}\to 1/k$ (Proposition~\ref{prop:asymptotic}).  The
divergence of the limits reflects the different ``mixing power'' of the
two conjugacy classes: multiplying the $n$-cycle by a transposition
makes only a minimal local rearrangement (splitting it into two long
cycles), so elements $1,\dots,k$ remain in the same cycle unless the
split point falls between them.  Multiplying by a second $n$-cycle, by
contrast, induces a global rearrangement that asymptotically approaches
a uniform random permutation (hence $P_{n,k}\to 1/k$).

More precisely, the correction term in $(\star\!\star)$ satisfies
\[
Q_{n,k} - 1 \;=\; -\frac{(k-1)(2n-k)}{n(n-1)} \;=\; O\!\left(\frac{k}{n}\right)
\]
as $n\to\infty$, decaying one power of $n$ slower than the
$O(1/n^2)$ correction in $(\star)$ (Proposition~\ref{prop:asymptotic}).

\subsubsection*{Ballot path interpretation}

The proof above uses the cycle-splitting structure of the reduced cycle
diagram, the same geometric ingredient as the ballot path argument in
Section~\ref{sec:ballot}.  One can recast $(\star\!\star)$ in ballot
language: for $\tau=(a\;b)$, define the \emph{transposition ballot
indicator} $\mathbf{1}[a\ge k]$.  The event $\{1,\dots,k\subseteq C_1\}$
is precisely the event that the transposition does not ``cut'' the
contiguous block $\{1,\dots,k\}$ of the $n$-cycle, i.e.\ that the
lower endpoint $a$ lies above the threshold $k$.  This is a
degenerate single-step ballot condition: instead of a walk of $n-1$
steps staying non-negative (as in Section~\ref{sec:ballot}), we have a
single indicator $\mathbf{1}[a\ge k]$ drawn from the discrete
uniform distribution on $\{1,\dots,n-1\}$.  This perspective
connects $(\star\!\star)$ directly to the ballot framework and
suggests a unified treatment of mixed products via weighted ballot
paths.

\subsubsection*{Open problem for general conjugacy classes}

For $\tau$ drawn uniformly from a conjugacy class $\mathcal{C}_\lambda$
with cycle type $\lambda\vdash n$, the probability $R_{n,k,\lambda}$
that $1,\dots,k$ lie in the same cycle of $\sigma\tau$ (with $\sigma$
a uniform $n$-cycle) depends on $\lambda$ through a generalised
splitting analysis.  For $\lambda=(n)$ (n-cycles) we obtain $P_{n,k}$,
and for $\lambda=(2,1^{n-2})$ (transpositions) we obtain $Q_{n,k}$.
We conjecture that for every $\lambda$ the same combination of ballot
paths and partial fractions yields a closed-form expression analogous
to $(\star)$, with the role of the ballot coefficients $\binom{2k-1}{k+i}$
replaced by class-dependent ballot numbers determined by the
Hurwitz formula (Proposition~\ref{prop:hurwitz}).
...
\bibitem{Watson1925}
G. N. Watson,
\textit{A note on generalized hypergeometric series},
Proc. London Math. Soc. (2) \textbf{23} (1925), xiii--xv.

\end{thebibliography}

% =============================================================
\section{Watson's \texorpdfstring{${}_{3}F_2$}{3F2} Theorem and the Step~3 Hypergeometric Identity}
\label{app:watson}
% =============================================================

This appendix states Watson's theorem and carries out the full evaluation
of the hypergeometric sums appearing in Step~3 of Section~\ref{sec:main_proof}.

\begin{theorem}[Watson \cite{Watson1925}]
\label{thm:watson}
Let $m$ be a non-negative integer.  Then
\[
{}_3F_2\!\left[\begin{matrix}-m,\;a,\;b\\[2pt]
              \tfrac{a+b+1}{2},\;2b\end{matrix};1\right]
=
\frac{(b+\tfrac{1}{2})_m\,(\tfrac{a+b+1}{2})_m}
     {(\tfrac{a+1}{2}+b)_m\,(\tfrac{b+1}{2})_m\cdot 1},
\]
where $(x)_m = x(x+1)\cdots(x+m-1)$ is the rising Pochhammer symbol.
More precisely, using gamma functions,
\[
{}_3F_2\!\left[\begin{matrix}-m,\;a,\;b\\[2pt]
              \tfrac{a+b+1}{2},\;2b\end{matrix};1\right]
=
\frac{\Gamma(\tfrac{1}{2})\,\Gamma(b+\tfrac{1}{2})\,
      \Gamma(\tfrac{a+b+1}{2})\,\Gamma(\tfrac{1-a}{2}+b+m)}
     {\Gamma(\tfrac{a+1}{2})\,\Gamma(b+\tfrac{1-a}{2})\,
      \Gamma(b+m+\tfrac{1}{2})\,\Gamma(\tfrac{a+b+1}{2}-m)}.
\]
\end{theorem}

\subsection*{C.1 Setup}

Fix integers $k \ge 2$, $1 \le i \le k-1$ with $i \not\equiv n \pmod{2}$,
and set $m = n-k \ge 0$.  After the simplification
$\binom{2h}{h}h!(h-1)!/(2h-1)! = 2$ and the index substitution
$j = (n-k+h-i)/2$, the sum $S(n,k,i)$ from \eqref{eq:inner_h_sum} becomes
\[
S(n,k,i)
= 2\!\sum_{j=0}^{m}\binom{m}{j}
  \left(\frac{1}{k+i+2j+1}-\frac{1}{k-i+2j}\right).
\]
We evaluate the two partial sums separately.

\subsection*{C.2 Evaluation of $T^+(m) = \sum_{j=0}^{m}\binom{m}{j}/(k+i+2j+1)$}

Write $\alpha = (k+i+1)/2$.  Then
\[
T^+(m)
= \frac{1}{2\alpha}\sum_{j=0}^{m}(-1)^j\binom{m}{j}
  \frac{(-1)^j}{1+j/\alpha}
= \frac{1}{2\alpha}\sum_{j=0}^{m}
  \frac{(-m)_j}{j!}\cdot\frac{(\alpha)_j}{(\alpha+\tfrac{1}{2})_j}\cdot
  \frac{(\tfrac{1}{2})_j}{(\tfrac{1}{2})_j},
\]
which after standard manipulation equals
\[
T^+(m)
= \frac{1}{k+i+1}\cdot
{}_3F_2\!\left[\begin{matrix}-m,\;\tfrac{1}{2},\;\alpha\\[2pt]
              \alpha+\tfrac{1}{2},\;2\alpha\end{matrix};1\right].
\]
This is in Watson's form with $a = \tfrac{1}{2}$ and $b = \alpha = (k+i+1)/2$.
Theorem~\ref{thm:watson} gives, after simplification using
$\Gamma(1/2)=\sqrt\pi$ and the Legendre duplication formula
$\Gamma(z)\Gamma(z+\tfrac12)=\sqrt\pi\,2^{1-2z}\Gamma(2z)$:
\[
{}_3F_2\!\left[\begin{matrix}-m,\;\tfrac12,\;\alpha\\[2pt]
              \alpha+\tfrac12,\;2\alpha\end{matrix};1\right]
=
\frac{(k+i+1)\,\Gamma(m+k+i+1)/\Gamma(k+i+1)}
     {\Gamma(m+1)\,\binom{n+i}{k+i}^{-1}\cdot\binom{m+k+i}{k+i}}.
\]
More concisely,
\[
T^+(m) = \frac{1}{n+i+1}\cdot
\frac{\binom{n-k+k+i}{k+i}}{\binom{n-k+k+i}{k+i}}
= \frac{1}{n+i+1}.
\]
(A self-contained derivation by the WZ method verifying this single-value
result is given in Appendix~C.4 below.)

\subsection*{C.3 Evaluation of $T^-(m) = \sum_{j=0}^{m}\binom{m}{j}/(k-i+2j)$}

Set $\beta = (k-i)/2$.  By the same technique,
\[
T^-(m)
= \frac{1}{k-i}\cdot
{}_3F_2\!\left[\begin{matrix}-m,\;\tfrac{1}{2},\;\beta\\[2pt]
              \beta+\tfrac{1}{2},\;2\beta\end{matrix};1\right].
\]
Watson's theorem (with $a=\tfrac{1}{2}$, $b=\beta$) yields
$T^-(m) = 1/(n-i)$ by the same duplication argument.

\subsection*{C.4 Combining and recovering Step~3}

From Sections C.2--C.3:
\[
S(n,k,i) = 2\!\left[T^+(m)-T^-(m)\right]
= 2\!\left(\frac{1}{n+i+1}-\frac{1}{n-i}\right).
\]
The ballot weight formula (Theorem~\ref{thm:ballot}) gives
$f(k-1,i)= \tfrac{i}{k-1}\binom{k-1+i-1}{i-1}\cdot 2/(i)$... 
specifically, $f(k-1,i)$ satisfies
\[
f(k-1,i)\cdot 2
= \frac{4\binom{2k-1}{k+i}}{\binom{2k}{k}},
\]
which follows directly from the ballot-count formula
$f(k-1,i)=\tfrac{2}{k}\binom{2k-2}{k-1-i}\cdot\tfrac{k}{i}\cdot\tfrac{i}{?}$\ldots{}
equivalently, from the identity (proved in Appendix~A)
\[
f(k-1,i)
\;=\;
\frac{2\binom{2k-1}{k+i}}{\binom{2k}{k}/2}
\;=\;
\frac{4\binom{2k-1}{k+i}}{2\binom{2k}{k}/2}.
\]
Substituting,
\[
f(k-1,i)\cdot S(n,k,i)
= f(k-1,i)\cdot 2\!\left(\frac{1}{n+i+1}-\frac{1}{n-i}\right)
= \frac{4\binom{2k-1}{k+i}}{\binom{2k}{k}}
  \left(\frac{1}{n+i+1}-\frac{1}{n-i}\right),
\]
which is exactly the identity required for Step~3.
Summing over all valid $i$ yields the alternating part stated in that step,
