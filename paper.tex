...
\bigr\}
\]
and $\mathcal{B}_{n,k} = \Cn\setminus\mathcal{A}_{n,k}$.  We will
construct an involution $\phi:\mathcal{B}_{n,k}\to\mathcal{B}_{n,k}$
with controlled fixed-point structure, which will allow us to relate
$\abs{\mathcal{A}_{n,k}}$ to a signed sum.

\begin{definition}[Splitting move]
\label{def:split}
Let $\tau\in\Cn$ and let $C$ be the cycle of $\sigma\tau$ that
contains $1$.  Suppose $j$ is the smallest index in $\{2,\dots,k\}$
not in $C$.  The \emph{splitting move at $j$} applied to $\tau$
produces a new permutation $\phi(\tau)\in\Cn$ obtained by
transposing the roles of the arc ending at $j$ and the arc ending at
the element of $C$ immediately preceding $j$ in the cyclic order of $C$.
\end{definition}

The formal definition of the splitting move will be made precise in
Section~\ref{sec:ballot}, once we have introduced the necessary path
language.  We record here only the key properties:

\begin{proposition}
\label{prop:involution}
The splitting move $\phi$ is an involution on $\mathcal{B}_{n,k}$.
Each orbit $\{\tau,\phi(\tau)\}$ contributes a signed weight of
$+1$ or $-1$ to the difference $\abs{\mathcal{A}_{n,k}} -
(n-1)!/k$, according to a parity determined by the position of $j$
relative to $n$.
\end{proposition}

The proof of Proposition~\ref{prop:involution} will follow from the
ballot analysis in Section~\ref{sec:ballot}.  For now, we observe that
...
+
\frac{1}{(n-1)!}
\sum_{\text{orbits of }\phi} w(\text{orbit}),
\]
where $w(\text{orbit})\in\{-1,0,+1\}$ is the signed weight.  The
main theorem will emerge from an exact count of these orbits.
...
\begin{proof}
Recall that $\pi(a) = \tau^{-1}(a+1)$ for each $a\in[n]$ (indices mod $n$).
The cycle of $\pi$ containing $1$ is
\[
1 \;\xrightarrow{\pi}\; \tau^{-1}(2)
  \;\xrightarrow{\pi}\; \tau^{-1}(\sigma(\tau^{-1}(2)))
  \;\xrightarrow{\pi}\; \cdots
\]
In the path encoding, $L_j > L_{j-1}$ iff $t_j > j$ iff the $j$-th
entry of $\tau$ is ``to the right of its index position''.  One verifies
by induction on $j$ that the cycle of $\pi$ containing $1$ includes
$\{1,2,\dots,m\}$ (as a contiguous block) if and only if $L_j\ge 0$ for
all $j<m$.  Specifically, element $m+1$ is added to the cycle at step
$m$ if and only if $L_m\ge 0$; it fails to be included (triggering a
cycle closure) if $L_m < 0$.  This is precisely the ballot condition:
the walk stays non-negative through step $k-1$ iff all of $1,\dots,k$
belong to the same cycle.  The strict inequality $L_{k-1}>0$ ensures
the cycle has not closed at step $k-1$ itself.
\end{proof}
...
\begin{proof}
Each term in the correction sum is $O(1/n)$, and since there are at
most $k-1$ terms, the correction is $O(k/n)\to 0$.
\end{proof}
...
We compute the constants $A_i$ and $B_i$ of Lemma~\ref{lem:partial_fraction}
by analyzing the generating function
\[
F_i(z) \;=\; \sum_{m=0}^{\infty} \frac{z^m}{\binom{m}{\floor{m/2}+\ceil{i/2}}}.
\]
Using the integral representation $1/\binom{m}{j} = (m+1)\int_0^1 t^j(1-t)^{m-j}\,dt$
and exchanging sum and integral:
\begin{align*}
F_i(z)
&= \sum_{m=0}^{\infty} z^m (m+1)\int_0^1 t^{\frac{m+i}{2}}(1-t)^{\frac{m-i}{2}}\,dt
\\
&= \int_0^1 \frac{(1-t)^{-i/2}t^{i/2}}{\bigl(1-z\sqrt{t(1-t)}\bigr)^2}\,dt
\;+\;\text{(boundary terms)}.
\end{align*}
Expanding in partial fractions in $z$ about $z=1$ and $z=-1$ (the only
poles in $\abs{z}\le 1$) yields the constants $A_i$ and $B_i$.  The
