...
The alternating double sum over $(i,h)$ is a convolution of the
ballot prefix weights $f(k-1,i)$ with the free suffix counts
$F(n-k,i,h)$ against the alternating kernel derived from
Lemma~\ref{lem:partial_fraction}.  By the generating-function
analysis of Appendix~B (specifically the residue computations in
Sections B.1--B.4), this convolution evaluates to
\[
\sum_{\substack{i=1\\i\not\equiv n\pmod{2}}}^{k-1}
\frac{4\,\binom{2k-1}{k+i}}{\binom{2k}{k}}
\left(\frac{1}{n+i+1}-\frac{1}{n-i}\right).
\]
The key step is that the $h$-convolution with the ballot suffix maps
the terminal-height poles at $h$ to intermediate-height poles at $i$
by the reflection principle for ballot paths, as established in the
contour integral of Section~B.2.
